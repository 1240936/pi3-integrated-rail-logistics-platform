...
BEGIN
    ...
    IF v_count > 0 THEN
        RAISE_APPLICATION_ERROR(-20002, 'Model name ''' || p_model_name || ''' already exists. Please choose a different name.');
    END IF;
...
EXCEPTION
    WHEN DUP_VAL_ON_INDEX THEN
        ...
        IF SQLERRM LIKE '%VEHICLEMODEL_GAUGE%' THEN
            RAISE_APPLICATION_ERROR(-20006, 'Gauge ID ' || v_gauge_id || ' is already associated with model ID ' || p_model_id || '.');
        ELSE
            RAISE;
        END IF;
    WHEN OTHERS THEN
...
BEGIN
    ...
    ELSE
        ...
        DBMS_OUTPUT.PUT_LINE('Model: ' || v_model_exists || ', Specs: ' || v_specs_exists || ', Electrical: ' || v_electrical_exists || ', Gauge: ' || v_gauge_exists);
    END IF;
...
BEGIN
    ...
    SELECT ID, modelName INTO v_existing_model_id, v_model_name
    FROM VehicleModel
    WHERE EXISTS (
        SELECT 1 FROM LocomotiveSpecs ls 
...
BEGIN
    ...
    SELECT modelName INTO v_existing_model_name
    FROM VehicleModel
    WHERE ROWNUM = 1;
