...
BEGIN
    ...
    IF p_route_id IS NOT NULL THEN
        BEGIN
            ...
            SELECT StartFacilityID, EndFacilityID INTO v_route_start_id, v_route_end_id
            FROM Route
            WHERE ID = p_route_id;
            ...
            BEGIN
                ...
                ELSE
                    SELECT MIN(seqNumber) INTO v_target_seq
                    FROM Path
                    WHERE RouteID = p_route_id AND FacilityID = p_facility_id2;
                END IF;
                ...
                WHEN NO_DATA_FOUND THEN
                    ...
                    BEGIN
                        ...
                        SELECT DISTINCT 
                            ...
                            END INTO v_connected_facility_id
                        FROM RailLine rl
                        ...
                        AND ROWNUM = 1;  -- Take first connection found
                        ...
                        ELSE
                            SELECT MIN(seqNumber) INTO v_target_seq
                            FROM Path
                            WHERE RouteID = p_route_id AND FacilityID = v_connected_facility_id;
                        END IF;
                        
                        -- Calculate direct connection distance from facility2 to connected facility
                        SELECT COALESCE(SUM(ls.length) / 1000.0, 0) INTO v_direct_connection_distance
                        FROM RailLine rl
            ...
            SELECT COALESCE(SUM(ls.length) / 1000.0, 0) INTO v_distance
            FROM (
                -- Get all route segments (facility pairs) up to the target facility
                -- Segment 1: Route start -> First path point
                SELECT 
                    v_route_start_id AS from_fac,
                    (SELECT FacilityID FROM Path 
                ...
                WHERE p.RouteID = p_route_id
                  ...
                  AND (v_connected_facility_id IS NOT NULL OR p_facility_id2 != v_route_end_id)
            ) route_segments
            JOIN RailLine rl ON (
...
BEGIN
    ...
    USING (
        SELECT LocomotiveID, RouteEndFacilityID
        FROM (
            SELECT AL.LocomotiveID, r.EndFacilityID AS RouteEndFacilityID,
        ...
        FROM (
            SELECT 
                ...
                ROW_NUMBER() OVER (PARTITION BY locos.ID ORDER BY 
                    CASE WHEN locos.RouteID IS NOT NULL AND locos.IsInTransitAtRequestedTime = 1 THEN 1 ELSE 2 END,  -- In-transit at requested time first
            ...
            FROM (
                ...
                WHERE NOT EXISTS (
                    ...
                    SELECT 1 FROM Assigned_Locomotive AL2
                ...
                SELECT
                    ...
                    CALCULATE_DISTANCE_BETWEEN_FACILITIES(v_start_facility_id, l.InitialFacilityID, p_route_id) AS DistanceFromStartKm,
                    0 AS IsInTransitAtRequestedTime
                FROM Locomotive l
                         INNER JOIN LocomotiveSpecs ls ON l.VehicleModelID = ls.VehicleModelID
                ...
                WHERE pl2.LocomotiveID IS NULL  -- Not in Parked_Locomotive
                  AND NOT EXISTS (
                    -- Exclude if locomotive is assigned to a planned train that overlaps with requested time
                    SELECT 1 FROM Assigned_Locomotive AL3
                  ...
                  AND NOT EXISTS (
                    ...
                    SELECT 1 FROM Assigned_Locomotive AL4
...
BEGIN
    ...
    USING (
        SELECT WagonID, DeliveryFacilityID
        FROM (
            SELECT AW.WagonID,
        ...
        FROM (
            SELECT 
                ...
                ROW_NUMBER() OVER (PARTITION BY wagons.ID ORDER BY 
                    CASE WHEN wagons.RouteID IS NOT NULL AND wagons.IsInTransitAtRequestedTime = 1 THEN 1 ELSE 2 END,  -- In-transit at requested time first
            ...
            FROM (
                ...
                WHERE NOT EXISTS (
                    ...
                    SELECT 1 FROM Assigned_Wagon AW2
                ...
                WHERE pw2.WagonID IS NULL  -- Not in Parked_Wagon
                  AND NOT EXISTS (
                    -- Exclude if wagon is assigned to a planned train at the requested time
                    SELECT 1 FROM Assigned_Wagon AW3
                  ...
                  AND NOT EXISTS (
                    -- Exclude if already shown as in-transit
                    SELECT 1 FROM Assigned_Wagon AW4
...
BEGIN
    ...
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RAISE_APPLICATION_ERROR(-20003, 'Planned train with RouteID ' || p_route_id || ' and start date ' || TO_CHAR(p_start_date, 'YYYY-MM-DD HH24:MI:SS') || ' does not exist.');
    END;
