...
BEGIN
    ...
    IF INSERTING THEN
        ...
        WHERE AW.PlannedTrainID = :NEW.PlannedTrainID
          AND AW.PlannedTrainStartDate = :NEW.PlannedTrainStartDate;
    ...
    WHERE AL.PlannedTrainID = :NEW.PlannedTrainID
      AND AL.PlannedTrainStartDate = :NEW.PlannedTrainStartDate;
...
BEGIN
    ...
    INTO v_loco_id
    FROM Locomotive L
    WHERE L.ID NOT IN (SELECT LocomotiveID
...
BEGIN
    ...
    INTO v_wagon_id
    FROM Wagon W
    WHERE W.ID NOT IN (SELECT WagonID
                       FROM Assigned_Wagon
                       WHERE PlannedTrainID = v_train_id
                         AND PlannedTrainStartDate = v_start_date)
      AND ROWNUM = 1;
