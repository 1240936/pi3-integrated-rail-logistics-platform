...
-- 2. Count existing wagons for this model AFTER cleanup (before registration)
    ...
    IF v_result = 1 THEN
        ...
        ELSE
            ...
            SELECT COUNT(*)
            INTO v_wagon_count_after
            FROM Wagon
            WHERE VehicleModelID = v_vehicle_model_id;

            DBMS_OUTPUT.PUT_LINE('Wagons before: ' || v_wagon_count_before || ', Wagons after: ' ||
                                 v_wagon_count_after);
            ...
            ELSE
                ...
                DBMS_OUTPUT.PUT_LINE('Expected count: ' || (v_wagon_count_before + 1) || ', Actual count: ' ||
                                     v_wagon_count_after);
            END IF;
        END IF;
